...
\StringTok{         }\KeywordTok{summarise}\NormalTok{(}\KeywordTok{sum}\NormalTok{(val_present))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{pago2}\OperatorTok{$}\NormalTok{factor_desc <-}\StringTok{ }\DecValTok{1} \OperatorTok{/}\StringTok{ }\NormalTok{(}\DecValTok{1} \OperatorTok{+}\StringTok{ }\NormalTok{r)}\OperatorTok{^}\NormalTok{(pago2}\OperatorTok{$}\NormalTok{t2)}
\CommentTok{# Calculamos el valor presente}
\NormalTok{pago2}\OperatorTok{$}\NormalTok{val_present <-}\StringTok{  }\NormalTok{pago}\OperatorTok{$}\NormalTok{pv_factor}\OperatorTok{*}\NormalTok{pago2}\OperatorTok{$}\NormalTok{pago}
...
Ahora se contruirá una función para valorizar cualquiee bono que pague
cupones iguales:
...
\NormalTok{precio.bono <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(p,tc,t,y)\{}
  \CommentTok{# rep returns a vector with value = p * r and times = ttm -1 }
...
{\centering \includegraphics{bookdown-demo_files/figure-latex/unnamed-chunk-39-1} 

}

\caption{Relación Bono vs Yield}\label{fig:unnamed-chunk-39}
\end{figure}
...
{\centering \includegraphics{bookdown-demo_files/figure-latex/unnamed-chunk-41-1} 
...
{\centering \includegraphics{bookdown-demo_files/figure-latex/unnamed-chunk-42-1} 

}

\caption{yield del tesoro de los Estados Unidos con ggplot2}\label{fig:unnamed-chunk-42}
\end{figure}
